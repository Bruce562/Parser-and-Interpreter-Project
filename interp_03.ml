    ...
    let rec go (pL : top_prog) = 
      (match pL with
        ...
        | (id, args, exp)::xs ->
          ...
          | Bool x -> Bool x 
          | Var v -> Var v 
          | Uop (uop, xp) -> Uop (uop, (go (id, idL, xp)::xs))
          | Bop (bop, xp1, xp2) -> Bop (bop, go (id, idL, xp1)::xs, go (id, idL, xp2)::xs)
          | Fun (l, xp) -> Fun (l, (go (id, idL, xp)::xs))
          | App (xp1, xp2) -> App ((go (id, idL, xp1)::xs), (go (id, idL, xp2)::xs))
          | Let (id1, args, xp1, xp2) -> App (Fun (id1, go (id,args, xp1)::xs), App ())
          | Ife (xp1, xp2, xp3) -> Ife ((go (id, idL, xp1)::xs), (go (id, idL, xp2)::xs), (go (id, idL, xp3)::xs))
          | Trace xp -> Trace (go ((id, idL, xp)::xs))
          )
      )
    in go l
